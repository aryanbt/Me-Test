    ...
    if (isset($_POST['upload'])) {
        ...
        db()->prepare('INSERT INTO media(title,description,tags,file_path,mime_type,media_type,uploaded_by) VALUES(:title,:description,:tags,:file_path,:mime_type,:media_type,:uploaded_by)')
            ->execute([
                ...
                'tags' => $tags,
                'file_path' => app_config('uploads.public_prefix') . $filename,
                'mime_type' => $mime,
            ...
            <?php foreach ($media as $item): ?>
                <article class="media-row">
                    <?php if ($item['media_type'] === 'video'): ?>
                        <video src="<?= h($item['file_path']) ?>" controls preload="metadata"></video>
                    <?php else: ?>
                        <img src="<?= h($item['file_path']) ?>" alt="<?= h($item['title']) ?>">
                    <?php endif; ?>
                    <div>
